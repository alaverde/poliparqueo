...
$(document).ready(function(){
    ...
    if(localStorage.getItem("permisos") == "2"){
        usuario = parseInt(localStorage.getItem("index"));
    }
    ...
    $.ajax({
        ...
        data: "metodo=consultarHistorial&usuario=" + usuario,
        success: function(response){
            if(response.result == true){
                historial.items = response.historial;
    ...
    $("#btn-reporte").click(function(){
        var myTestXML = new myExcelXML(historial.items);
        myTestXML.downLoad();
    });
